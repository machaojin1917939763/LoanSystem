    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="学年" prop="academicYearId">
          <el-input v-model="form.academicYearId" placeholder="请输入学年id" />
        </el-form-item>
...
<script>
import { listTime, getTime, delTime, addTime, updateTime } from "@/api/student/time";
...
export default {
  ...
  methods: {
    randomDate(academicYearId) {
      // 去掉'年'字
      let yearStr = academicYearId.replace('年', '')
      // 根据学年生成一个开始日期和结束日期
      let start = new Date(yearStr + '-01-01')
      let end = new Date(yearStr + '-12-31')
      // 生成一个随机的毫秒数
      let randomMs = Math.floor(Math.random() * (end - start + 1)) + start
      // 创建一个新的日期对象
      let date = new Date(randomMs)
      // 格式化日期为 yyyy-MM-dd
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除放款时间编号为"' + timeIds + '"的数据项？').then(function() {
